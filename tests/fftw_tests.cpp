...
protected:
	...
	{
		testing::mock_fftw::set_mock(&mock);
	}

	virtual ~FFTWTest()
	{
		testing::mock_fftw::set_mock(nullptr);
	}

	using test_type = double;

	// Mock variables
	testing::mock_fftw mock; 
	...
	fftw_plan mock_plan = reinterpret_cast<fftw_plan>(1);

	// Enforce call sequence
	InSequence s;
};

TEST_F(FFTWTest, VectorCallsMallocAndFree) 
{
	const size_t alloc_size = alloc_num * sizeof(test_type);
	
	EXPECT_CALL(mock, fftw_malloc(alloc_size))
		.Times(1)
		.WillOnce(Return(real_input.data()));
	EXPECT_CALL(mock, fftw_malloc(2 * alloc_size))
		.Times(1)
		.WillOnce(Return(complex_input.data()));
	EXPECT_CALL(mock, fftw_free(complex_input.data())).Times(1);
	EXPECT_CALL(mock, fftw_free(real_input.data())).Times(1);

	fftw::vector<test_type> vec(alloc_num);
	fftw::vector<std::complex<test_type>> vec_complex(alloc_num);
}

TEST_F(FFTWTest, BasicInterfaceC2C)
{	
	EXPECT_CALL(mock, fftw_plan_many_dft(
		Eq(extent.size()), extent.data(), 1,
		reinterpret_cast<fftw_complex*>(complex_input.data()), nullptr, 1, 1,
		reinterpret_cast<fftw_complex*>(complex_output.data()), nullptr, 1, 1,
		static_cast<int>(sign), flags))
		.Times(1)
		.WillOnce(Return(mock_plan));
	EXPECT_CALL(mock, fftw_execute(mock_plan)).Times(1);
	EXPECT_CALL(mock, fftw_execute_dft(mock_plan,
		reinterpret_cast<fftw_complex*>(new_complex_input.data()),
		reinterpret_cast<fftw_complex*>(new_complex_output.data()))).Times(1);
	EXPECT_CALL(mock, fftw_destroy_plan(mock_plan)).Times(1);
...
TEST_F(FFTWTest, BasicInterfaceC2R)
{	
	EXPECT_CALL(mock, fftw_plan_many_dft_c2r(
		Eq(extent.size()), extent.data(), 1,
		reinterpret_cast<fftw_complex*>(complex_input.data()), nullptr, 1, 1,
		...
		.WillOnce(Return(mock_plan));
	EXPECT_CALL(mock, fftw_execute(mock_plan)).Times(1);
	EXPECT_CALL(mock, fftw_execute_dft_c2r(mock_plan,
		reinterpret_cast<fftw_complex*>(new_complex_input.data()),
...
TEST_F(FFTWTest, BasicInterfaceR2C)
{	
	EXPECT_CALL(mock, fftw_plan_many_dft_r2c(
		Eq(extent.size()), extent.data(), 1,
		...
		.WillOnce(Return(mock_plan));
	EXPECT_CALL(mock, fftw_execute(mock_plan)).Times(1);
	EXPECT_CALL(mock, fftw_execute_dft_r2c(mock_plan,
		new_real_input.data(),
		reinterpret_cast<fftw_complex*>(new_complex_output.data()))).Times(1);
	EXPECT_CALL(mock, fftw_destroy_plan(mock_plan)).Times(1);
...
TEST_F(FFTWTest, BasicInterfaceR2R)
{
	EXPECT_CALL(mock, fftw_plan_many_r2r(
		Eq(extent.size()), extent.data(), 1,
		real_input.data(), nullptr, 1, 1,
		real_output.data(), nullptr, 1, 1,
		kind.data(), flags))
		...
		.WillOnce(Return(mock_plan));
	EXPECT_CALL(mock, fftw_execute(mock_plan)).Times(1);
	EXPECT_CALL(mock, fftw_execute_r2r(mock_plan,
		new_real_input.data(),
		new_real_output.data())).Times(1);
	EXPECT_CALL(mock, fftw_destroy_plan(mock_plan)).Times(1);
...
TEST_F(FFTWTest, AdvancedInterfaceC2C)
{
	EXPECT_CALL(mock, fftw_plan_many_dft(
		Eq(extent.size()), extent.data(), num_batches,
		reinterpret_cast<fftw_complex*>(complex_input.data()), inembed.data(), input_stride, input_dist,
		reinterpret_cast<fftw_complex*>(complex_output.data()), onembed.data(), output_stride, output_dist,
		static_cast<int>(sign), flags))
		.Times(1)
		.WillOnce(Return(mock_plan));
	EXPECT_CALL(mock, fftw_execute(mock_plan)).Times(1);
...
TEST_F(FFTWTest, AdvancedInterfaceC2R)
{
	EXPECT_CALL(mock, fftw_plan_many_dft_c2r(
		Eq(extent.size()), extent.data(), num_batches,
		reinterpret_cast<fftw_complex*>(complex_input.data()), inembed.data(), input_stride, input_dist,
...
TEST_F(FFTWTest, AdvancedInterfaceR2C)
{
	EXPECT_CALL(mock, fftw_plan_many_dft_r2c(
		Eq(extent.size()), extent.data(), num_batches,
...
TEST_F(FFTWTest, AdvancedInterfaceR2R)
{
	EXPECT_CALL(mock, fftw_plan_many_r2r(
		Eq(extent.size()), extent.data(), num_batches,
		real_input.data(), inembed.data(), input_stride, input_dist,
...
TEST_F(FFTWTest, GuruInterfaceC2C)
{
	EXPECT_CALL(mock, fftw_plan_guru64_dft(
		Eq(rank_params.size()), rank_params.data(), Eq(batch_params.size()), batch_params.data(),
		reinterpret_cast<fftw_complex*>(complex_input.data()),
		reinterpret_cast<fftw_complex*>(complex_output.data()),
		static_cast<int>(sign), flags))
		.Times(1)
		.WillOnce(Return(mock_plan));
	EXPECT_CALL(mock, fftw_execute(mock_plan)).Times(1);
...
TEST_F(FFTWTest, GuruInterfaceC2CSplit)
{
	EXPECT_CALL(mock, fftw_plan_guru64_split_dft(
		Eq(rank_params.size()), rank_params.data(), Eq(batch_params.size()), batch_params.data(),
		...
		.WillOnce(Return(mock_plan));
	EXPECT_CALL(mock, fftw_execute(mock_plan)).Times(1);
	EXPECT_CALL(mock, fftw_execute_split_dft(mock_plan,
...
TEST_F(FFTWTest, GuruInterfaceC2R)
{
	EXPECT_CALL(mock, fftw_plan_guru64_dft_c2r(
		Eq(rank_params.size()), rank_params.data(), Eq(batch_params.size()), batch_params.data(),
		reinterpret_cast<fftw_complex*>(complex_input.data()),
		real_output.data(), 
		flags))
...
TEST_F(FFTWTest, GuruInterfaceC2RSplit)
{
	EXPECT_CALL(mock, fftw_plan_guru64_split_dft_c2r(
		Eq(rank_params.size()), rank_params.data(), Eq(batch_params.size()), batch_params.data(),
		...
		.WillOnce(Return(mock_plan));
	EXPECT_CALL(mock, fftw_execute(mock_plan)).Times(1);
	EXPECT_CALL(mock, fftw_execute_split_dft_c2r(mock_plan,
...
TEST_F(FFTWTest, GuruInterfaceR2C)
{
	EXPECT_CALL(mock, fftw_plan_guru64_dft_r2c(
		Eq(rank_params.size()), rank_params.data(), Eq(batch_params.size()), batch_params.data(),
		real_input.data(),
		reinterpret_cast<fftw_complex*>(complex_output.data()),
		flags))
...
TEST_F(FFTWTest, GuruInterfaceR2CSplit)
{
	EXPECT_CALL(mock, fftw_plan_guru64_split_dft_r2c(
		Eq(rank_params.size()), rank_params.data(), Eq(batch_params.size()), batch_params.data(),
		...
		.WillOnce(Return(mock_plan));
	EXPECT_CALL(mock, fftw_execute(mock_plan)).Times(1);
	EXPECT_CALL(mock, fftw_execute_split_dft_r2c(mock_plan,
...
TEST_F(FFTWTest, GuruInterfaceR2R)
{
	EXPECT_CALL(mock, fftw_plan_guru64_r2r(
		Eq(rank_params.size()), rank_params.data(), Eq(batch_params.size()), batch_params.data(),
		real_input.data(),
		real_output.data(),
		kind.data(), flags))
